# PLTR — Palantir Technologies

**Layer:** 10-Applications (AI Platform / Enterprise Software)
**Last reviewed:** 2026-05-17
**Current conviction:** {✗ / ? / ✓ / ✓✓ / ✓✓✓}
**Current position size:** {0% / X% of portfolio}
**Thesis-break trigger:** {what single development would force exit}

> **[STALE - revisit]** This thesis file is still the blank template. Q1 2026 earnings (May 4, 2026) delivered $1.633B revenue (+85% YoY), 60% adjusted operating margin, 150% NRR, and raised FY26 guidance to $7.65-7.66B. A full thesis write-up is needed. See `transcripts/Q1-2026.md` and `transcripts/Q1-2026-delta.md` for the complete earnings data.

---

## 1. One-line thesis

> Owned as the operating system for enterprise and government AI: its Ontology and AIP platform turn AI into production decisions on top of deeply entrenched federal contracts, making Palantir an AI infrastructure peer rather than a downstream app consumer.

---

## 2. Position in the AI supply chain [STALE - revisit]

- **What they do:** {2-3 sentences}
- **Where they sit:** {layer in supply chain, role}
- **Unique vs. commoditized:** {what's defensible about this position}
- **AI revenue mix:** {% of revenue tied to AI infrastructure spend, citation}
> **Q1 2026 update:** AIP is now driving 133% US commercial growth. Management frames PLTR as the only production-grade AI orchestration platform. Revenue is ~100% AI-linked given AIP/Foundry/Gotham are the entire product suite. Need to populate this section.

---

## 3. Customers

| Customer | % revenue | Source / date |
|---|---|---|
| | | |

- **Hyperscaler exposure:** {direct sales to MSFT/GOOGL/AMZN/META/ORCL, %}
- **Concentration risk:** {top 3 customers as % of revenue}
- **Substitution risk:** {can customers in-house? Vertical integration risk?}

---

## 4. Pricing & demand

- **Pricing trend:** {ASPs rising/flat/falling, last 8 quarters}
- **Lead times / backlog:** {are they widening or compressing?}
- **Gross margin trend:** {last 8 quarters, with citation to filings}
- **Capacity utilization:** {if disclosed}

---

## 5. Financials snapshot [STALE - revisit]

(See `financials.xlsx` for detail)

| Metric | TTM | YoY change | 3-yr CAGR |
|---|---|---|---|
| Revenue | | | |
| Gross margin | | | |
| Operating margin | | | |
| FCF | | | |
| FCF margin | | | |
| Capex | | | |
| Capex / revenue | | | |
| Net debt | | | |
| Net debt / EBITDA | | | |
| Share count YoY | | | |

> **Q1 2026 snapshot:** Revenue $1.633B (Q1, +85% YoY); Adj gross margin 88%; Adj operating margin 60%; Adj FCF $925M (57% margin); Cash $8.0B, no debt; ARR run rate ~$6.5B; SBC $202M (12.4% of rev). Full TTM table needs population from 10-Q.

---

## 6. Moat

- **IP / process know-how:** {patents, trade secrets, technical lead}
- **Scale advantage:** {unit cost vs. competitors at volume}
- **Switching costs:** {how locked-in are customers}
- **Network effects:** {usually NA for hardware; relevant for software}
- **Regulatory / geographic:** {utility territory, fab location, etc.}

**Moat rating:** {Wide / Narrow / None}

---

## 7. Valuation [STALE - revisit]

- **Current multiple:** {P/E, EV/EBITDA, EV/Sales — pick the right one for the business}
- **Peer comparison:** {how does it trade vs. comparables}
- **Bull case multiple (3-5 yr):** {if AI capex compounds}
- **Bear case multiple (3-5 yr):** {if hyperscaler capex flatlines}
- **Implied return at current price:** {bull / base / bear scenarios}
- **What's priced in vs. consensus:** {is consensus too high or too low}
> **Q1 2026 update:** Forward P/E ~97x; trailing P/E ~155x; market cap ~$350B on ~$6.5B ARR (54x revenue). Even at 71% guided growth, the stock prices in years of sustained hypergrowth. Valuation is the binding constraint on the score — see sensitivity analysis. Needs full peer comp (CRM, NOW, SNOW, DDOG) and scenario modeling.

---

## 8. Catalysts (next 4 quarters)

| Date | Event | Why it matters |
|---|---|---|
| | Earnings | |
| | Product launch | |
| | Customer announcement | |
| | Capacity online | |
| | Industry conference | |

---

## 9. Risks

- **Thesis-killer #1:** {single development that would force exit}
- **Thesis-killer #2:** {second-most concerning risk}
- **China / export controls:** {direct or indirect exposure}
- **Cyclical risk:** {what happens in a hyperscaler capex pause}
- **Technology substitution:** {what could make this product obsolete}
- **Regulatory / political:** {permitting, antitrust, tariffs}
- **Execution / management:** {any specific concerns}

---

## 10. The "inverse the thesis" test

> What would convince me I'm wrong? If I cannot answer this in two specific, falsifiable statements, I do not understand the position well enough to own it.

1. {Specific falsifiable statement #1}
2. {Specific falsifiable statement #2}

---

## Source log

All sources used to populate this file. Every claim above should trace to one of these.

- 2026-05-04 — 8-K / Press release — Palantir Q1 2026 earnings press release (SEC EDGAR)
- 2026-05-04 — Earnings call transcript — [Motley Fool](https://www.fool.com/earnings/call-transcripts/2026/05/04/palantir-pltr-q1-2026-earnings-transcript/)
- 2026-05-04 — Earnings call transcript — [Insider Monkey](https://www.insidermonkey.com/blog/palantir-technologies-inc-nasdaqpltr-q1-2026-earnings-call-transcript-1754013/)
- 2026-05-26 — Trade press / analysis — [Investing.com Q1 slides analysis](https://www.investing.com/news/company-news/palantir-q1-2026-slides-us-revenue-tops-100-growth-rule-of-40-hits-145-93CH-4657648)
- 2026-05-26 — Trade press / analysis — [SaaStr Q1 analysis](https://www.saastr.com/palantir-q1-2026-has-broken-the-enterprise-software-mold-again-revenue-accelerates-to-85-at-6b-arr/)
- 2026-05-26 — Trade press / analysis — [Futurum Group](https://futurumgroup.com/insights/palantir-q1-fy-2026-revenue-beats-estimates-us-demand-drives-outlook-raise/)
- {Date} — ...

---

## Decision log

| Date | Action | Conviction | Rationale | Position size after |
|---|---|---|---|---|
| | Initiated coverage | | | |
| | | | | |

