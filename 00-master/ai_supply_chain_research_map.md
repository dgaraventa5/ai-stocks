...
### Consumer fintech (AI adopter)
- **Robinhood (HOOD)** — ⚠️ **AI-adopter, no AI-revenue line.** Retail brokerage; growth engine is prediction markets / event contracts, margin lending, equities, and Gold subscriptions. AI angle: **Cortex** Gold assistant + internal-efficiency ML, and — the strongest piece — **agentic finance** (Agentic Trading + Agentic Credit Card + MCP-server access for third-party AI agents, launched 2026-05-27): a real *agentic-finance rails* optionality bet, but monetization is unchanged (transaction/interchange/net-interest), so still **no AI revenue line** and still **demand-side** (destination for others' agents, not an AI supplier). Forward-thesis is TSLA-style optionality (AI ≈ 0% of P&L today). Fails the AI-materiality *revenue* gate that dropped it from the Layer 11 experiment; **re-added to Layer 10 at Dom's request 2026-07-08** as a tracked name — see per-stock/HOOD/context-2026-07-08.md. Reg overhang: prediction-markets gambling-law challenges (its top growth driver)
